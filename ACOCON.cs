...
public class ACOCON
{
    ...
    private float CalculateTotalPheromoneAndVisibility(List<ACOConnection> ConnectionsFromNodeAndNotVisited)
    {
        ...
        foreach (ACOConnection aConnection in ConnectionsFromNodeAndNotVisited)
        {
            ...
            (Mathf.Pow(aConnection.PheromoneLevel, Alpha) * Mathf.Pow((1 / aConnection.Distance), Beta));
        }
    ...
    {
        string Output = " Last Ant Tour Info (Q: " + Q + ", Alpha: " + Alpha + ", Beta: " + Beta + ",EvaporationFactor: " + EvaporationFactor + ", DefaultPheromone: " + DefaultPheromone + "):\n";
        for (int i = 0; i < Ants.Count; i++)
        {
            Output += "Ant " + i + " - Start Node: " + Ants[i].StartNode.name + " | Tour Length: " + Ants[i].AntTourLength + "\n";
        }
        Debug.Log(Output);
    }
}
